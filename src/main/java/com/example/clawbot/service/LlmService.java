 ...
 *   <li><b>工具调用</b>：支持天气查询、新闻热榜、地理编码、周边搜索、路线规划、语音合成、塔罗占卜等工具</li>
 ...
 * @see com.example.clawbot.tool.NewsTool
...
public class LlmService {
     ...
     *   <li>在请求中注册全部 7 个工具定义，设置 {@code tool_choice: "auto"} 让模型自行决定是否调用</li>
     ...
     * 工具数量固定（7 个），if-else 链代码清晰、IDE 可追踪引用、
    ...
    private String executeTool(String functionName, String arguments, String userId) {
        ...
        if (newsTool.getToolName().equals(functionName)) {
            return newsTool.execute(functionName, arguments);
        }
